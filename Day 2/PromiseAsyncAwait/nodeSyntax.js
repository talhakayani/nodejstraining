...
const task2 = false;
const task3 = true;
const taskOnePromis = new Promise((resolve, reject) => {
  setTimeout(() => {
    task1 ? resolve("task 1 completed") : reject("task 1 fails");
  }, 1000);
});
const taskTwoPromis = new Promise((resolve, reject) => {
  setTimeout(() => {
    task2 ? resolve("task 2 completed") : reject("task 2 fails");
  }, 2000);
});
const taskThreePromis = new Promise((resolve, reject) => {
  setTimeout(() => {
...
Promise.all([taskOnePromis, taskTwoPromis, taskThreePromis])
  .then((messages) => console.log(...messages))
  .catch((messages) => console.log(messages));
/// this will execute and return the first promice when success or fails
Promise.race([taskOnePromis, taskTwoPromis, taskThreePromis])
